...
export class UpdateInforPage implements OnInit {
  ...
  constructor(private camera: Camera,private platform: Platform,private domSanitizer:DomSanitizer,public photoService: PhotoService,public actionSheetController: ActionSheetController,public service : ProvidersService,public navCtrl: NavController, public http: HttpClient, public toast: ToastController) { }
  news: any = {};
  imageData: string;
  @Input() useURI = true;
  ngOnInit() {
  ...
  resign() {
    ...
    console.log(this.news);
    if(this.news.newsname != '' && this.news.email != '' && this.news.password != ''){
      this.http.post(this.service.server+'post.php', this.news)
    .subscribe(data => {
      console.log(data);
      let result = data;
      console.log(result);
